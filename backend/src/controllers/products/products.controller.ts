...
import {
  ApproveProductDto,
  RejectProductDto,
} from '../../dtos/products/approve-product.dto';
...
export class ProductsController {
  ...
  async posSearch(
    ...
    const result = await this.getProductsService.getForPOS(
      user.companyId,
      search,
    );
  ...
  async getPendingApprovals(@CurrentUser() user: RequestUser) {
    const result = await this.getProductsService.getPendingApprovals(
      user.companyId,
    );
    return { success: true, data: result };
  }
  ...
  async approve(
    ...
    const result = await this.approveProductService.approve(
      user.companyId,
      id,
      dto,
      user.id,
  ...
  async reject(
    ...
    const result = await this.approveProductService.reject(
      user.companyId,
      id,
      dto,
      user.id,
