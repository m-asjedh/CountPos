...
export class SuppliersController {
  ...
  async update(@CurrentUser() user: RequestUser, @Param('id') id: string, @Body() dto: UpdateSupplierDto) {
    await this.prisma.supplier.updateMany({ where: { id, companyId: user.companyId }, data: dto });
    const result = await this.prisma.supplier.findFirst({ where: { id, companyId: user.companyId } });
    return { success: true, data: result };
  }
  ...
  async delete(@CurrentUser() user: RequestUser, @Param('id') id: string) {
    await this.prisma.supplier.updateMany({ where: { id, companyId: user.companyId }, data: { isActive: false } });
